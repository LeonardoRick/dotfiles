 ...
 */
local ReplaceCtrlCmdRules() = std.map(ReplaceCtrlCmdItem, ['f', 'j', 'l', 'n', 'o', 'p', 'r', 's', 't', 'x', 'slash']);
/**
 * list all the binds I want to replace only one direction: Ctrl+A -> Cmd+A. Both shortcuts do the same thing
 */
local ReplaceCtrlCmdOneDirectionRules() = std.map(ReplaceCtrlCmdItemOneDirection, ['a', 'k','c', 'v', 'w', 'z', 'spacebar']);
/**
